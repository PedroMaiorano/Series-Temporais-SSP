            ...
            fluidPage(
              ...
              if (tema() == "IML (Instituto Médico Legal)") {
                ...
                if (is.null(dado7())) {
                  ...
                  dado7(fread("www/iml_parcial.csv"))
                ...
                if (is.null(dado3())) {
                  ...
                  dado3(fread("www/latrocinio_parcial.csv"))
                }
                ...
                if (is.null(dado4())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 1)
                  dado4(fread("www/lesaoCSM_parcial.csv"))
                }
                ...
                if (is.null(dado11())) {
                  ...
                  dado11(fread("www/rouboVeiculo_parcial.csv"))
                  dado11(  dado11() %>%
                ...
                if (is.null(dado12())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 10)
                  showNotification()
                  dado12(fread("www/furtoVeiculo_parcial.csv"))
                  dado12(  dado12() %>%
                             mutate(data_bo_reg = as.Date(datahora_bo_reg),
                                    nome_municipio_circ=cidade_ocorr))
                ...
                if (is.null(dado5())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 1)
                  dado5(fread("www/morteDIP_parcial.csv"))
                }
                ...
                if (is.null(dado6())) {
                  ...
                  dado6(fread("www/morteSuspeita_parcial.csv"))
                ...
                if (is.null(dado8())) {
                  ...
                  dado8(fread("www/dadosCriminais_parcial.csv", header = TRUE))
                }
                ...
                if (is.null(dado9())) {
                  ...
                  dado9(fread("www/rouboCelular_parcial.csv"))
                  dado9(  dado9() %>%
                            mutate(data_bo_reg = as.Date(datahora_bo_reg),
                                   nome_municipio_circ=cidade_ocorr
                            ))
                ...
                if (is.null(dado10())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 20)
                  dado10(fread("www/furtoCelular_parcial.csv"))
                  dado10(  dado10() %>%
                ...
                if (is.null(dado1())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 1)
                  dado1(fread("www/homicidioDoloso_parcial.csv"))
                }
              ...
              ")
                ...
                if (is.null(dado2())) {
                  showNotification("Carregando os dados. Aguarde!",type = "message",duration = 1)
                  dado2(fread("www/feminicidio_parcial.csv"))
                             ...
                             fluidPage(
                               fluidRow(
                                 ...
                                 column(width =4,
                                            box(title = "Objetivos",
                                                width = NULL,
                                                "Essa plataforma visa tornar acessível a análise de dados, permitino que pessoas de diferentes
                                                campos e níveis explorem os temas de maneira intuitiva e personalizada. Como resultado
                                                a teoria é transportada para a pática.")),
                                 column(width = 4,
                               ...
                               fluidRow(
                                 column(2,
                                        verticalLayout(
                                          ...
                                          fluidRow(
                                            column(6,numericInput("serie_ordem", "Ordem", 1, min = 1, max = 3, step = NA))),
                                          ),
                                          box(width = NULL,
                                 ...
                                 column(2,
                                        verticalLayout(
                                          box(width = NULL,
                                              ...
                                              div(
                                                tags$ul(
                                               lapply(c("ADF","KPSS","Cox_Stuart","Kendall","Wavk","Sazonal"), function(bolinha){
                               ...
                               fluidRow(
                                 column(2,
                                        verticalLayout(
                                          ...
                                          fluidRow(column(6,sliderInput("pzin", "p", value = c(0,3), min = 0, max = 3, step = NA,sep = c(0,1,2,3),ticks = FALSE)),
                                                   column(6,sliderInput("pzao", "P", value = c(0,2), min = 0, max = 2, step = NA,sep = c(0,1,2),ticks = FALSE))),
                                          fluidRow(column(6,sliderInput("qzin", "q", value = c(0,3), min = 0, max = 3, step = NA,sep = c(0,1,2,3),ticks = FALSE)),
                                                   column(6,sliderInput("qzao", "Q", value = c(0,2), min = 0, max = 2, step = NA,sep = c(0,1,2),ticks = FALSE))),
                                          fluidRow(column(6,sliderInput("dzin", "d", value = c(0,2), min = 0, max = 2, step = NA,sep = c(1,2),ticks = FALSE)),
                                                   column(6,sliderInput("dzao", "D", value = c(0,1), min = 0, max = 1, step = NA,sep = c(0,1),ticks = FALSE))),
                                 ...
                                 column(2,
                                        verticalLayout(
                                          box(width = NULL,
                                              title ="Predição",
                                              column(6,
                                              numericInput("lag_pred","lag futuro", value = 12, min = 0, max = 52))),
                                          box(width = NULL,
                                              title = "Testes",
...
#
      ...
      observeEvent(input$gerar_modelo,{
       ...
       # updateSliderInput(session, "pzin", value = c(armazem_ordem1()+armazem_ordem2(),armazem_ordem1()+armazem_ordem2()))
       # updateSliderInput(session, "qzin", value = c(armazem_lag2(),armazem_lag2()))
        if (armazem_diff1()){
          updateSliderInput(session, "dzin", value = c(armazem_ordem1(),armazem_ordem1()))}
        if (armazem_diff2()){
          updateSliderInput(session, "dzao", value = c(1,1))}
        estado("decomp")
